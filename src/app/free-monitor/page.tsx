...
export default function FreeMonitorPage() {
  ...
  useEffect(() => {
    fetch('/api/free-monitor-schedule')
      .then(r => r.json())
  ...
  const startPolling = useCallback(() => {
    ...
    pollingRef.current = setInterval(async () => {
      try {
        const data = await fetch('/api/free-course-monitor').then(r => r.json());
        setLogs(data.logs ?? []);
        if (!data.running) {
          setRunning(false);
  ...
  async function handleRun() {
    ...
    const data = await fetch('/api/free-course-monitor', { method: 'POST' })
      .then(r => r.json())
      .catch(() => null);
  ...
  async function saveSchedule() {
    ...
    setScheduleMsg(null);
    try {
      const res = await fetch('/api/free-monitor-schedule', {
        method: 'POST',
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">🔍 무료강의 모니터링</h1>
          <p className="text-gray-500 mt-1">9개 플랫폼의 새로운 무료강의를 확인하고 노션에 저장해요</p>
        </div>
        <button
      ...
      {resumedFromBg && running && (
        <div className="mb-4 px-4 py-3 bg-blue-50 border border-blue-200 rounded-xl text-sm text-blue-700 flex items-center gap-2">
          ...
          페이지를 나갔다 와도 모니터링이 백그라운드에서 계속 실행 중입니다.
        </div>
      )}
        ...
        <>
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-blue-50 border border-blue-200 rounded-2xl p-5 text-center">
              <p className="text-sm text-blue-600 font-medium">새로 저장</p>
              <p className="text-3xl font-bold text-blue-700 mt-1">{result.newCount}건</p>
            </div>
            <div className="bg-gray-50 border border-gray-200 rounded-2xl p-5 text-center">
              <p className="text-sm text-gray-600 font-medium">중복 건너뜀</p>
              <p className="text-3xl font-bold text-gray-700 mt-1">{result.skippedCount}건</p>
            </div>
            <div className="bg-red-50 border border-red-200 rounded-2xl p-5 text-center">
              <p className="text-sm text-red-600 font-medium">오류</p>
              <p className="text-3xl font-bold text-red-700 mt-1">{result.errorCount}건</p>
            </div>
          </div>
          ...
          {result.newCourses.length > 0 && (
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden mb-6">
              <div className="px-6 py-4 border-b border-gray-100">
                <h2 className="text-lg font-semibold text-gray-900">새로 저장된 강의</h2>
              </div>
                ...
                <table className="w-full">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-5 py-3 text-left text-xs font-medium text-gray-500">강의 제목</th>
                      <th className="px-5 py-3 text-center text-xs font-medium text-gray-500">링크</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100">
                    {result.newCourses.map((course, i) => (
                      <tr key={i} className="hover:bg-gray-50">
                        <td className="px-5 py-4 text-sm font-medium text-gray-900">{course.platform}</td>
                        <td className="px-5 py-4 text-sm text-gray-600">{course.instructor}</td>
                        <td className="px-5 py-4 text-sm text-gray-600 max-w-xs truncate" title={course.courseTitle}>{course.courseTitle}</td>
                        <td className="px-5 py-4 text-center">
                          <a href={course.url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800" title="강의 페이지 열기">🔗</a>
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          {result.errorItems.length > 0 && (
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-100">
                <h2 className="text-lg font-semibold text-gray-900">오류 항목</h2>
              </div>
              <div className="divide-y divide-gray-100">
                {result.errorItems.map((item, i) => (
                  <div key={i} className="px-6 py-4 flex items-start gap-3">
                    <span className="text-sm font-medium text-gray-900 flex-shrink-0">{item.platform}</span>
          ...
          <div className="relative w-full max-w-md bg-white h-full shadow-2xl flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5">
              ...
              <p className="text-xs text-gray-500 mb-4">
                설정한 시간에 자동으로 무료강의 모니터링을 실행합니다.
                <br />
                <span className="text-orange-600">배포 후 cron-job.org에서 <code>/api/free-monitor-schedule/check</code>를 1분마다 호출 필요</span>
              </p>
